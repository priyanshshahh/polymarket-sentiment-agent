...
export default function App() {
  ...
  return (
    <div className="min-h-screen text-sm">
      <header className="border-b border-edge bg-panel/50 sticky top-0 backdrop-blur z-30">
        <div className="max-w-[1400px] mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-2.5 h-2.5 rounded-full bg-accent shadow-[0_0_10px_#7cf6c4]" />
            <div className="font-semibold tracking-wide">DOA · COMMAND CENTER</div>
            {status && (
              <>
                <Pill tone={status.mode === "LIVE" ? "negative" : "neutral"}>
                  {status.mode}
                </Pill>
      ...
      {err && (
        <div className="bg-danger/10 text-danger text-xs px-6 py-2 border-b border-danger/30">
          {err}
        </div>
      )}

      <main className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
        {/* Top row: portfolio stats + equity curve */}
          ...
          <Panel title="Portfolio" className="lg:col-span-1">
            {portfolio && (
              <div className="grid grid-cols-2 gap-y-4">
                <Stat
                  label="Equity"
                  value={`$${portfolio.total_equity_usdc.toFixed(2)}`}
                  tone="neutral"
                />
                <Stat
                  label="24h PnL"
                  value={`${portfolio.daily_pnl_usdc >= 0 ? "+" : ""}$${portfolio.daily_pnl_usdc.toFixed(
                    2
                  )}`}
                ...
                <Stat
                  label="Realized"
                  value={`${portfolio.realized_pnl_usdc >= 0 ? "+" : ""}$${portfolio.realized_pnl_usdc.toFixed(
                    2
                  )}`}
                ...
                <Stat
                  label="Unrealized"
                  value={`${portfolio.unrealized_pnl_usdc >= 0 ? "+" : ""}$${portfolio.unrealized_pnl_usdc.toFixed(
                    2
                  )}`}
                ...
                <Stat
                  label="Open size"
                  value={`$${portfolio.open_positions_usdc.toFixed(2)}`}
                />
        ...
        <Panel
          ...
          right={
            <span className="text-xs text-muted">
              {trades.length} total · click for rationale
            </span>
          }
        >
          {trades.length === 0 ? (
            <div className="text-muted text-xs">
              No trades yet. The Overseer is gating on edge ≥{" "}
              {status?.edge_threshold.toFixed(2)} and confidence ≥{" "}
              {status?.min_signal_confidence.toFixed(2)}.
            </div>
          ) : (
                    ...
                    >
                      ...
                      <td
                        className={`pr-4 text-right ${
                          t.edge > 0 ? "text-accent" : "text-danger"
                        }`}
                      >
                        {t.edge > 0 ? "+" : ""}
                        {t.edge.toFixed(3)}
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Panel title="Watched markets (Oracle)">
            {markets.length === 0 ? (
                  ...
                  <li key={m.id} className="py-2 text-xs">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex gap-2 flex-shrink-0">
                        <Pill>{m.outcome}</Pill>
                        <Pill tone={m.price > 0.5 ? "positive" : "neutral"}>
                          {m.price.toFixed(3)}
                        </Pill>
                      </div>
                    </div>
                    <div className="text-muted mt-1">
                      bid {m.best_bid.toFixed(3)} · ask {m.best_ask.toFixed(3)} · liq $
                      {m.liquidity.toFixed(0)} · 24h ${m.volume_24h.toFixed(0)}
                    </div>
                  </li>
                ))}
              </ul>
        ...
        {status && (
          <Panel title="Risk config (read-only)">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
              <Stat label="Edge threshold" value={status.edge_threshold.toFixed(3)} />
              <Stat label="Min confidence" value={status.min_signal_confidence.toFixed(2)} />
              <Stat label="Max trade" value={`$${status.max_usdc_per_trade.toFixed(0)}`} />
              <Stat
                label="Daily drawdown"
                value={`$${status.daily_drawdown_usdc.toFixed(0)}`}
                tone="warn"
              />
            </div>
          </Panel>
      ...
      <footer className="text-center text-muted text-[10px] py-6">
        DOA Agent · paper trading by default · {status?.watched_markets ?? 0} markets watched ·
        last cycle {status?.last_loop_at ? new Date(status.last_loop_at).toLocaleTimeString() : "—"}
      </footer>
    </div>
  );
}
