...
SQLite by default (single file, simple and durable for a single process).
Production can point DATABASE_URL at Postgres (e.g. Neon free tier) instead
-- Render's free-tier disk is ephemeral, so SQLite there survives only
